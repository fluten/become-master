...
举个具体的例子。某个周二下午你听说周末要参加一个红酒晚宴，你的酒类知识仅限于「干红和半干红有啥区别」。下面是从焦虑到凯旋的完整旅程——同时也是这个 skill 跑起来时发生在你身上的故事：

```mermaid
journey
    title 用户旅程 · 从社交焦虑到老炮儿凯旋
    section 危机降临
      听说周末有红酒晚宴: 2: 你
      担心在内行面前露怯: 1: 你
    section 求助 Claude
      告诉 Claude 我想直接成为红酒大师: 3: 你
      收到完整速成包 1-10 模块: 4: 你
      惊喜发现品牌鄙视链有懂哥之选: 5: 你
    section 临场前 30 秒
      读模块 11 速记卡的 18 个节点: 4: 你
      默背 5 个高频行话: 4: 你
    section 晚宴现场
      抛出第一个行话被点头认可: 5: 你
      被追问细节差点卡壳: 2: 你
...
最戏剧性的是「晚宴现场」段：高峰开局 → 突然卡壳跌到底 → 救场话术拉回 → 物理逃离反杀 → 反客为主收尾。这正好把 skill 最有特色的几个模块（行话、救场话术、物理逃离、反客为主）按时间顺序串成一个故事。
